...
exports.getCampaignPublic = getCampaignPublic;
...
function getAllCampaigns(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const where = {
                status: {
                    in: [enums_1.CampaignStatus.LIVE, enums_1.CampaignStatus.COMPLETED],
                },
            };
            ...
            if (minAmount || maxAmount) {
                where.targetAmount = {};
                if (minAmount)
                    where.targetAmount.gte = Number(minAmount);
                if (maxAmount)
                    where.targetAmount.lte = Number(maxAmount);
            }
            let orderBy = { createdAt: 'desc' };
            if (sort === 'oldest')
                orderBy = { createdAt: 'asc' };
            else if (sort === 'target_high')
                orderBy = { targetAmount: 'desc' };
            else if (sort === 'target_low')
            ...
            const baseUrl = getBaseUrl(req);
            let formattedCampaigns = campaigns.map(c => (Object.assign(Object.assign({}, c), { totalMoneyRaised: c.moneyDonations.reduce((sum, d) => sum + d.amount.toNumber(), 0), coverImage: `${baseUrl}/uploads/${c.coverImage}`, proofLinks: (0, file_1.convertProofLinksToUrls)((0, file_1.parseProofLinks)(c.proofLinks), baseUrl) })));
            if (sort === 'most_funded') {
...
function getCampaignPublic(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const { id } = req.params;
            ...
            return res.status(200).json({
                success: true,
                campaign: Object.assign(Object.assign({}, campaign), { coverImage: `${baseUrl}/uploads/${campaign.coverImage}`, proofLinks: (0, file_1.convertProofLinksToUrls)((0, file_1.parseProofLinks)(campaign.proofLinks), baseUrl), visits: campaign.visits, shareCount: campaign.shareCount })
            });
        }
        catch (error) {
