import type { PrismaClient, Campaign, MoneyDonation } from '../../../generated/prisma/client.js';

export interface RecommendationOptions {
  userId?: string;
  limit?: number;
  excludeDonated?: boolean;
}

export interface CampaignWithScore extends Campaign {
  recommendationScore: number;
  matchReasons: string[];
}

function calculateCampaignSimilarity(
  campaign1: { category?: string; targetAmount?: number },
  campaign2: { category?: string; targetAmount?: number }
): number {
  let score = 0;
...
async function getUserDonatedCampaigns(
  userId: string,
  prisma: PrismaClient
): Promise<Set<string>> {
  const donations = await prisma.moneyDonation.findMany({
...
async function getSimilarToPopularCampaigns(
  userId: string,
  prisma: PrismaClient,
  limit: number
): Promise<CampaignWithScore[]> {
  const userDonations = await prisma.moneyDonation.findMany({
    where: { donorId: userId },
    include: { campaign: true },
    take: 10,
  });
  ...
  const similarUserDonations = await prisma.moneyDonation.findMany({
    where: {
  ...
  const campaignScores = new Map<string, CampaignWithScore>();
  ...
  for (const donation of similarUserDonations) {
    ...
    if (!existing) {
      campaignScores.set(campaign.id, {
        ...campaign,
        recommendationScore: score,
        matchReasons: ['Popular among similar donors'],
      });
    } else {
...
export async function getRecommendations(
  ...
  const results: CampaignWithScore[] = [];
  ...
  if (!userId) {
    return liveCampaigns
      ...
      .map(c => ({
        ...
        matchReasons: ['Trending campaign'],
      }));
  }

  const donatedCampaigns = await getUserDonatedCampaigns(userId, prisma);
  const userCategories = await getUserCategoryPreferences(userId, prisma);

  for (const campaign of liveCampaigns) {
    if (excludeDonated && donatedCampaigns.has(campaign.id)) continue;
    ...
    for (const [donatedId, donated] of userCategories.entries()) {
      const similarity = calculateCampaignSimilarity(
        { category, targetAmount: Number(campaign.targetAmount) },
        { category: donatedId }
      );
      if (similarity > 0.5) {
        score += similarity * 30;
  ...
  const similarCampaigns = await getSimilarToPopularCampaigns(
    userId, prisma, limit - results.length
  );
...
export async function getSimilarCampaigns(
  ...
  const scoredCampaigns = liveCampaigns.map(campaign => ({
    campaign,
    similarity: calculateCampaignSimilarity(
      { category: sourceCampaign.category || 'general', targetAmount: Number(sourceCampaign.targetAmount) },
      { category: campaign.category || 'general', targetAmount: Number(campaign.targetAmount) }
    ),
  }));
  ...
  return scoredCampaigns
    ...
    .map(s => ({
      ...s.campaign,
      recommendationScore: Math.round(s.similarity * 100),
      matchReasons: ['Similar to campaign you\'re viewing'],
    }));
}
